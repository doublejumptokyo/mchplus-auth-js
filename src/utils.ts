...
export class Utils {
  ...
  get walletName(): string {
    ...
    if (!this.provider) return "noProvider";
    ...
    if (this.provider.isOpera) return "opera";

    // https://tokenpocket.jp/ja/
    if (this.provider.isTokenPocket) return "tokenPocket";
    ...
    if (this.provider.isDapper) return "dapper";

    // https://developer.samsung.com/blockchain/platform/programming-guide/cucumber-webview.html
    if (this.provider.isCucumber) return "samsung";

    // https://tokenpocket.jp/ja/
    if (this.provider.isTokenPocketCrypton) return "dedicatedTokenPocketApp";

    // https://alphawallet.com/
    if (this.provider.isAlpha) return "alphaWallet";

    // https://qurage.app/en
    if (this.provider.isQurage) return "qurage";

    // https://goldenwallet.io/
    if (this.provider.isGolden) return "golden";

    // https://techcrunch.com/2018/04/13/coinbase-acquires-decentralized-app-browser-wallet-cipher-browser/
    if (this.provider.isCipher) return "cipher";

    // https://www.myetherwallet.com/
    if (this.provider.isMew) return "mew";

    // https://token.im/
    if (this.provider.isImToken) return "imToken";

    // https://www.dapppocket.io/
    if (this.provider.isDappPocket) return "dappPocket";

    // https://www.alicedapp.com/
    if (this.provider.isAlice) return "alice";

    // https://alphawallet.com/
    if (this.provider.isAlphaWallet) return "alphaWallet";

    // https://www.coinomi.com/en/
    if (this.provider.isCoinomi) return "coinomi";

    // https://mykey.org/
    if (this.provider.isMYKEY) return "mykey";
